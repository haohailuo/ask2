    ...
    if (isset($this->post['submit'])) {
    	 $this->load("topic");
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_tmp_file)) {
                ...
                $_ENV['topic']->addtopic($title, $desrc, $filepath,$this->user['username'],$this->user['uid'],1,$acid);
               
                $this->message('添加成功！','user/addxinzhi');
            } else {
    ...
    function oneditimg() {
        if (isset($_FILES["userimage"])) {
            ...
            if (move_uploaded_file($_FILES["userimage"]["tmp_name"], $upload_tmp_file)) {
                ...
                if (image_resize($upload_tmp_file, ASK2_ROOT . $smallimg, 80, 80))
                    echo 'ok';
            }
        } else {
            if ($this->setting["ucenter_open"]) {
